...
if (
  isset($_POST['authenticator_get'], $_POST['authenticator_username'], $_POST['authenticator_pass'])
...
// Change passwords ?
if (
  isset($_POST['change_password_get'], $_POST['change_password_username'], $_POST['change_password_pass'])
  && !empty($_POST['change_password_pass'])
) {
  $req = $bdd->prepare('SELECT * FROM user WHERE user_id = ?');
  $req->execute(array($_POST['change_password_username']));
  $data = $req->fetch();

  // Error ?
  if ($data && passEqual($_POST['change_password_pass'], $data['user_pass'])) {

    $_SESSION['user_id'] = $data['user_id'];
    header('Location: /change_password_response.php');
  } else {
    $error = true;
  }
}
...
// Get the configuration files ?
if (
  isset($_POST['configuration_get'], $_POST['configuration_username'], $_POST['configuration_pass'], $_POST['configuration_os'])
...
<body class='container-fluid'>
  ...
  // --------------- CHANGE PASSWORD ---------------
    ...
    require(dirname(__FILE__) . '/include/html/menu.php');
    require(dirname(__FILE__) . '/include/html/form/change_password.php');
  }
    ...
    <nav class="navbar navbar-default">
      <div class="row col-md-12">
        ...
        <div class="col-md-6">
          ...
          <a class="navbar-text navbar-right" href="index.php?change_password" title="change-password"><button class="btn btn-default">Change Password</button></a>
          </p>
        </div>
      </div>
    </nav>
